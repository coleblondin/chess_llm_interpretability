...
def find_end_of_black_moves_without_checkormate_symbs(moves_string: str) -> list[int]:
    ...
    """
    indices = []
    for index, char in enumerate(moves_string):
        if char == ".":
            prev_black_move_end = moves_string[:index].rfind(" ")
            if prev_black_move_end != -1:
                if moves_string[prev_black_move_end-1] == "+":
                    # print(moves_string[:prev_black_move_end-1])
                    indices.append(prev_black_move_end-1)
                else:
                    # print(moves_string[:prev_black_move_end])
                    indices.append(prev_black_move_end)
            else:
                # indices.append("YEET")
                # Can't add this because maybe you're looking at the first move?
                pass
        else:
            # Don't add because only want to add 1 index per move
            pass
...
is_check_config = Config(
    min_val=0,
    max_val=1,
    custom_board_state_function=board_to_is_in_check,
